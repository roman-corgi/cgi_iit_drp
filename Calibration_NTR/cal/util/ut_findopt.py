...
from .findopt import find_optimum_1d, find_optimum_2d, QuadraticException
...
class TestFindoptFunctions(unittest.TestCase):
    ...
    def test_find_optimum_1d_scaled(self):
        """Test that a 1-D fit still passes for scaled down data."""
        xTrue = 4.87654
        scaleFac = 1e-16

        delta = 1.0
        xVec = np.arange(-4, 5, delta)
        arrayToFit = -(xVec - xTrue)**2 * scaleFac
    ...
    def test_find_optimum_2d_scaled(self):
        """Test that a 1-D fit still passes for scaled down data."""
        xTrue = 4.87654
        yTrue = 3.24689
        scaleFac = 1e-8
        ...
        yVec = np.arange(-5, 3, delta)
        X, Y = np.meshgrid(xVec, yVec)
        arrayToFit = ((X - xTrue)**2 + (Y - yTrue)**2) * scaleFac
