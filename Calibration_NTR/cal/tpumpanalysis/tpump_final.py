...
from cal.util.gsw_nonlin import get_relgains

from cal.tpumpanalysis.trap_id import illumination_correction, trap_id
from cal.tpumpanalysis.trap_fitting import trap_fit, trap_fit_const, fit_cs
...
def tpump_analysis(base_dir, time_head, emgain_head,
    num_pumps, meta_path, nonlin_path, mean_field = None, length_lim = 6,
    ...
    meta_path : str
        ...
        not used.  (See sample_data below for more information.)
    nonlin_path : str or None, optional
        Full path of residual nonlinearity CSV file to correct for nonlinearity
        (if needed).  If not needed, let input be None (which is the default
        value).
    ...
    try:
        ...
        raise FileNotFoundError('metadata.yaml not found')
            ...
            for sch_dir in sorted(os.listdir(sch_dir_path)):
                ...
                for file in os.listdir(curr_sch_path):
                    ...
                    if os.path.isfile(f) and f.endswith('.fits') and \
                        ...
                        prows, pcols, r0c0 = meta._imaging_area_geom()
                        if prows > np.shape(data)[0] or \
                            pcols > np.shape(data)[1] or \
                            r0c0[0] > np.shape(data)[0] or \
                            r0c0[1] > np.shape(data)[1]:
                            raise TPumpAnException('metadata file inconsistent'
                            ' with frame')
                        # Get prescan region
                        prescan = meta.slice_section(data, 'prescan')
                        ...
                        if nonlin_path != None:
                            try:
                                d *= get_relgains(d, em_gain, nonlin_path)
                            except:
                                raise FileNotFoundError('nonlinearity file '
                                'not found')
                        d = d/em_gain
                        frames.append(d)
...
if __name__ == '__main__':
    ...
    meta_path_full = Path(here, '..', 'util', 'metadata.yaml')
    ...
    emgain_head, num_pumps, meta_path_sub, nonlin_path = None,
